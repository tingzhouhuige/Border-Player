...
- 阻止保存 Windows 最小化时的屏外哨兵坐标，修复进程仍在运行但窗口无法找到的问题。
- 改进主屏、副屏和混合 DPI 环境下的窗口持久化；显示器未变化时按 DWM 可见边框物理像素精确恢复。
- 对显示器断开、分辨率变化、损坏坐标和超大窗口尺寸增加安全映射与居中回退。
- 串行化设置写入，避免窗口事件并发保存导致配置被旧状态覆盖。
- 修复启动恢复播放状态、重建媒体库时歌单重复及过期歌词源残留等生命周期问题。
- 修复全局热键注册时序，并将更新检查切换到本项目仓库及正确的语义版本比较。
- 移除质感模式中等宽度下左侧导航栏的独立半透明底板，使侧栏与整窗背景融合。
